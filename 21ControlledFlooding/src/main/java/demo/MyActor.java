...
public class MyActor extends UntypedAbstractActor{
	...
	private ArrayList<MyMessage> seenMessages;

	public MyActor() {
        contacts = new ArrayList<ActorRef>();
        seenMessages = new ArrayList<MyMessage>();
    }
	...
	public void onReceive(Object message) throws Throwable {
        ...
        if(message instanceof MyMessage) {
            ...
            for(MyMessage seen : seenMessages)
                if(seen == m) {
                    log.info("["+getSelf().path().name()+"] recognises this message from before.");
                    return;
                }
            seenMessages.add(m);
            for(ActorRef c : contacts)
